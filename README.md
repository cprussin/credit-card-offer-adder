...
if it is readable by anyone but its owner — which it can only check for a file
it opens itself, so if you later hand the job to agenix or sops-nix, the
permissions on their output are yours to get right.
...
Without a TPM, drop `--with-key=host+tpm2` for host-key-only sealing; without
