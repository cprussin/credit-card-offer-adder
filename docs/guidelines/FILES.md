...
declarations (schema definitions, fixtures, selector tables), slot them
into the order where they make sense — typically alongside helpers, since
they're in service of the entry point.
...
```ts
// wrong — barrel import
import { addOffers } from "@offers/offer-run";

// wrong — unnecessary chain (a → b → c → d)
import { addOffers } from "../offer-run/exports";

// correct — direct import from the defining module
import { addOffers } from "@offers/offer-run/add-offers";
```
